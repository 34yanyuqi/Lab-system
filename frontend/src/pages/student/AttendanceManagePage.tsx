...
import {
  ...
  WarningOutlined, MinusCircleOutlined, StopOutlined, EyeOutlined, DeleteOutlined
...
export default function StudentAttendanceManagePage() {
  ...
  const [permission, setPermission] = useState<{ can_upload: boolean; can_edit: boolean } | null>(null)
  const [permissionError, setPermissionError] = useState(false)

  const fetchPermission = useCallback(async () => {
    setPermissionError(false)
    try {
      const res = await attendanceApi.getMyPermission()
      setPermission(res.data.data)
    } catch {
      // 必须记录失败状态，否则页面会一直停留在"正在加载..."
  ...
  useEffect(() => {
    if (permission && (permission.can_upload || permission.can_edit)) {
      fetchGrades()
    }
  }, [fetchGrades, permission])
  ...
  }, [fetchOverview, fetchAnomalyRecords, permission])
  ...
  const barOption = useMemo(() => {
    if (!overview || !overview.students || !Array.isArray(overview.students) || overview.students.length === 0) return {}

    const validStudents = overview.students.filter(s => s && s.name)
    if (validStudents.length === 0) return {}

    const names = validStudents.map(s => s.name || '')
    const absentRateData = validStudents.map(s => {
    ...
    const lateRateData = validStudents.map(s => {
      const totalDays = s.totalDays || 0
  ...
  const columns: ColumnsType<AttendanceStudentSummary> = [
    {
      ...
      key: 'name',
      render: (name: string) => (
        <a onClick={() => navigate(`/student/attendance/detail/${encodeURIComponent(name)}`)}>
          {name}
        </a>
      )
    },
    ...
    },
    {
      ...
      render: (_: unknown, record: AttendanceStudentSummary) => (
        <Button
          type="link"
          icon={<EyeOutlined />}
          onClick={() => navigate(`/student/attendance/detail/${encodeURIComponent(record.name)}`)}
        >
          详情
        </Button>
      )
      ...
      <div className="page-shell">
        <Card>
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <StopOutlined style={{ fontSize: '48px', color: permissionError ? '#ff4d4f' : '#faad14' }} />
            <p style={{ marginTop: 16, fontSize: 16 }}>
              {permissionError ? '获取考勤权限失败，请稍后重试' : '正在加载...'}
            </p>
            {permissionError && (
              <Button type="primary" onClick={fetchPermission} style={{ marginTop: 12 }}>
                重新加载
              </Button>
            )}
          </div>
  ...
  if (!permission.can_upload && !permission.can_edit) {
    return (
      <div className="page-shell">
        <Card>
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            <StopOutlined style={{ fontSize: '48px', color: '#faad14' }} />
            <p style={{ marginTop: 16, fontSize: 16 }}>您暂无考勤管理权限，请联系导师授权</p>
          </div>
        </Card>
      </div>
    )
